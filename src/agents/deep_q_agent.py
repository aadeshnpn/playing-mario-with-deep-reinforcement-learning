...
class DeepQAgentRes1(Agent):
    """The Deep Q reinforcement learning algorithm.

    This agents exhibts the propert of resilient 1. This means whenever it applies
    the fire action, the command is simply not passed to the environment 50% of the time.
    We can change the probability and see the results.
    """
    ...
    def play(self, games: int=100, exploration_rate: float=0.05) -> np.ndarray:
        ...
        progress = tqdm(range(games), unit='game')
        progress.set_postfix(score='?')
        #print ('action space',self.env.action_space, dir(self.env.action_space))
        #exit()
        # a list to keep track of the scores
        scores = np.zeros(games)
        # iterate over the number of games
        for game in progress:
            ...
            while not done:
                # predict the best action based on the current state
                action = self.predict(state, exploration_rate)
                res_action_list = [3, 4, 8, 9] # 3 is right and fire, 4 is right jump fire,
                # 8 is left fire and 9 is left jump fire
                # Will will reduce these action to simpler counter parts

                #if np.random.rand() > 0.5 and action in res_action_list:
                if action in res_action_list:
                    # Lets try putting all actions into NOP (0)
...
__all__ = [DeepQAgent.__name__, DeepQAgentRes1.__name__]
